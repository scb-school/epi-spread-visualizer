...
class EpiSpread: 
    ...
    def _parse_columns(cls, df):
        ...
        """
        time_series_columns = []
        number_columns = [df.columns[i] for i in range(0,len(df.columns)) if isinstance(df.iloc[0,i], numpy.int64) or isinstance(df.iloc[0,i], numpy.float64)]
    ...
    @classmethod
    def _find_available_graphs(cls, number_columns, date_columns, iso_columns, time_series_columns):
        available_graphs = []
    ...
    def run_query(cls):
        ...
        print(cls._find_available_graphs(number_columns, date_columns, iso_columns, time_series_columns))
        ...
        if graph_type == "heat map w/ time slider" or graph_type == "heat map":
            if graph_type == "heat map w/ time slider":
                ...
                start_date = input("Which date would you like to start plotting from?\nPlease format it YYYY-MM-DD, and it cannot be earlier than " + df[time_series_column].iloc[0] + ".")
            ...
            time_series = input("Which time series would you like to plot?") 
            time_series_list = time_series.strip("]").strip("[").replace("'", "").replace(",", "").split(" ")
